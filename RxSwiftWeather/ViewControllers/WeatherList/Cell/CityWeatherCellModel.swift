...
extension CityWeatherCellModel {
    ...
    init?(with data: WeatherDetails?) {
        ...
        latitude = lat
        longitude = long
        icon = CityWeatherCellModel.weatherIconURL(for: iconName)
    }
    
    static func empty() -> CityWeatherCellModel {
        return CityWeatherCellModel(id: "0", city: "No city", temperature: "No data", temperatureCategory: .unknown, detail: "No details", longitude: 0, latitude: 0, icon: nil)
    }
